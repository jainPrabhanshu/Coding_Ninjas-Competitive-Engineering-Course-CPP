...
int magicGrid(int** arr,int m,int n){
    ...
    for(int i=n-3;i>=0;i--){
        if(arr[m-1][i+1]<0){
            dp[m-1][i] = dp[m-1][i+1] - arr[m-1][i+1];
        }else{
            dp[m-1][i] = max(1,dp[m-1][i+1] - arr[m-1][i+1]);
        }
    }
    for(int i=m-2;i>=0;i--){
        for(int j=n-2;j>=0;j--){
			int right,down;
            if(arr[i+1][j]<0){
                down = dp[i+1][j] -arr[i+1][j];
            }else{
                down = max(1,dp[i+1][j] -arr[i+1][j]);
            }
            if(arr[i][j+1]<0){
                right = dp[i][j+1]-arr[i][j+1];
            }
            else{
                right = max(1,dp[i][j+1] -arr[i][j+1]);
            }
...
int main(){
    ...
    while(t--){
        ...
        int ans = magicGrid(grid,row,col);
        cout<<ans<<endl;
    }
